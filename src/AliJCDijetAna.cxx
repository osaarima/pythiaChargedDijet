        ...
        if(jets[udijet].size()>1) {
            ...
            if(jets[udijet][0].pt()>fLeadingJetCut) {
                ...
                if(jets[udijet][1].pt()>fSubleadingJetCut) {
                    ...
                    dijets[udijet] = jets[udijet][0] + jets[udijet][1];
                    mjj = dijets[udijet].m();
                    ptpair = dijets[udijet].pt();
...
void AliJCDijetAna::CalculateResponse(AliJCDijetAna *anaDetMC, AliJCDijetHistos *fhistos) {
    ...
    for (ujet = 0; ujet < Njets; ujet++) { //True MC jets
        ...
        for (ujetDetMC = 0; ujetDetMC < NjetsDetMC; ujetDetMC++) { //Det MC jets
            if(DeltaR(jets[iRaw][ujet], jetsDetMC[iRaw][ujetDetMC]) < matchingR) {
                if(jetsDetMC[iRaw][ujetDetMC].pt() > maxpt) {
                    maxpt = jetsDetMC[iRaw][ujetDetMC].pt();
                    maxptIndex = ujetDetMC;
                    bfound = true;
                    if(ujet==0) bLeadingMatch   = true;
                    if(ujet==1) bSubleadingMatch= true;
                    cout << "found, detPt vs truePt: " << maxpt << " <> " << jets[iRaw][ujet].pt() << ", index: " << maxptIndex << endl;
                }
            }
        }
        if(bfound) {
            fhistos->fh_jetResponse->Fill(jetsDetMC[iRaw][maxptIndex].pt(), jets[iRaw][ujet].pt()); // Hit
            cout << "hit!!!!!!!!!" << endl;
        } else {
            //Fill miss
            //
            cout << "miss..." << endl;
            fhistos->fh_jetResponse->Fill(-1, jets[iRaw][ujet].pt()); // Miss
        }
    }


    vector<fastjet::PseudoJet> dijetsDetMC = anaDetMC->GetDijets();

    if(jets[iRaw].size()>1) {
        if(jets[iRaw][0].pt()>fLeadingJetCut && jets[iRaw][1].pt()>fSubleadingJetCut) {
            if(jetsDetMC[iRaw].size()>1) {
                if(jetsDetMC[iRaw][0].pt()>fLeadingJetCut && jetsDetMC[iRaw][1].pt()>fSubleadingJetCut) {
                    if(bLeadingMatch && bSubleadingMatch) {
                        fhistos->fh_dijetResponse->Fill(dijetsDetMC[iRaw].m(), dijets[iRaw].m()); // Hit
                    }
                } else {
                    fhistos->fh_dijetResponse->Fill(-1, dijets[iRaw].m()); // Miss
                }
            } else {
                fhistos->fh_dijetResponse->Fill(-1, dijets[iRaw].m()); // Miss
            }
        ...
        if(jetsDetMC[iRaw].size()>1) {
            if(jetsDetMC[iRaw][0].pt()>fLeadingJetCut && jetsDetMC[iRaw][1].pt()>fSubleadingJetCut) {
                fhistos->fh_dijetResponse->Fill(dijetsDetMC[iRaw].m(), -1); // Fake
            }
        }
    }
    return;
